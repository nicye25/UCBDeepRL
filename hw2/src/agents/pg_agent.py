...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        """
        T = len(rewards)
        discount_factors = self.gamma ** np.arange(T)
        total_discounted_return = np.dot(discount_factors, rewards)
        return np.full(T, total_discounted_return)
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                # A_GAE(s_t) = delta_t + gamma*lambda * A_GAE(s_{t+1})
                ...
                for i in reversed(range(batch_size)):
                    ...
                    next_value = values[i+1] * (1 - terminals[i])
                    delta = rewards[i] + self.gamma * next_value - values[i]
                    next_adv = advantages[i+1] * (1 - terminals[i])
